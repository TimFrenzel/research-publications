...
The mathematical foundation of RAG explains its effectiveness. Traditional language models generate responses by predicting the most probable next token based on training patterns. RAG instead retrieves relevant documents first, then conditions generation on actual data:

$$P(y|x) = \sum_{d \in D} P(y|x,d) \cdot P(d|x)$$

where $d$ represents retrieved documents from corpus $D$, and the model generates output $y$ conditioned on both query $x$ and retrieved evidence. This grounding mechanism transforms speculation into citation.
